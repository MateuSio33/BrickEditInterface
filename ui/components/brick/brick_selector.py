...
from ui.components.brick.filters import FilterMode, FilterResult, BaseFilter, ColorFilter
...
class BrickSelector(Widget):
    ...
    def is_allowed(self, brick: brickedit.Brick) -> bool:
        ...
        for f in self.filters:
            ...
            # Brick vetoed -> NEVER allow
            if filter_result == FilterResult.VETOED:
                return False
            # Brick allowed -> matches at least one filter. Will be allowed unless another filter vetoes. Keep checking other filters for any vetos
